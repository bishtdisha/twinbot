...
def build_langgraph_workflow():
    ...
    def process_node(input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Fields
        # Use LLM mapping to get user_term to key mapping
        api_field_result = select_fields_from_query(user_query, FIELD_LIST)
        ...
        # If the result is a list of dicts with 'key' and 'user_term', use it
        if api_field_result and isinstance(api_field_result[0], dict) and 'key' in api_field_result[0]:
            selected_keys = [item['key'] for item in api_field_result]
            user_term_map = {item['key']: item.get('user_term', FIELD_METADATA.get(item['key'], item['key'])) for item in api_field_result}
        else:
            # Fallback: treat as list of keys
            selected_keys = api_field_result
            user_term_map = {key: FIELD_METADATA.get(key, key) for key in selected_keys}
        ...
        # Dynamically estimate limit based on duration and fields
        duration_seconds = (end_dt - start_dt).total_seconds()
        records_per_minute = 1 / 5  # 1 record every 5 minutes
        estimated_records = int(duration_seconds / 60 * records_per_minute * max(len(selected_keys), 1))
        dynamic_limit = min(max(estimated_records, 1000), 100000)
        ...
        try:
            ...
            for key, values in api_response.items():
                if key not in expected_keys:
                    continue  # skip any extra/unwanted fields
            ...
            # If API returned no data, try fallback to CSV
            if final_df.empty:
                raise Exception("API returned no data, falling back to CSV.")

            # Generate comprehensive natural language answer, passing user_term_map
